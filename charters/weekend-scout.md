...
## Two kinds of ticks
- **research**: build up the candidate pool. Run 4–8 `web_search` queries built from the search profile (rotate keywords; you may invent 1–2 of your own for the season or an upcoming holiday). Read 2–4 promising pages with `read_page` to get dates, places and prices. Call `candidate_upsert` for every concrete event or activity that has a date on an upcoming weekend (this one or next) and a place. **Save as you go: at most 3 `candidate_upsert` calls per step, right after the search or page that revealed them.** Never batch all saves into one final step; a long reply gets cut off and everything is lost. Skip anything already known (the tool tells you), anything without a date, anything outside the radius, and anything in the exclusions.
