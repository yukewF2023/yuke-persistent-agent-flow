...
.tasks{list-style:none;margin:0;padding:0}.tasks li{border-bottom:1px solid var(--line)}.tasks li:last-child{border-bottom:0}.task{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:1px 12px;align-items:baseline;padding:7px 0;color:var(--fg)}.task:hover{text-decoration:none}.task:hover .t{color:var(--accent)}.task .t{font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.task .k{grid-column:1;font:11.5px var(--mono);color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.task .m{grid-column:2;grid-row:1/3;font-size:12px;color:var(--muted);text-align:right;white-space:nowrap;font-variant-numeric:tabular-nums}.task .live{grid-column:1/3;font-size:12px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
const taskRow = (t: { id: number; key: string; title: string; goal_id?: string | null }, meta: string, live = "") =>
  `<li><a class="task" href="/tasks/${t.id}"><span class="t">${esc(t.title)}</span><span class="m">${meta}</span><span class="k">${chip(t.key, t.goal_id)}${esc(t.key)}</span>${live ? `<span class="live">${live}</span>` : ""}</a></li>`;
